...
import { AUTH_ENDPOINTS, AUTH_EVENTS, AUTH_STORAGE_KEYS, TOKEN_EXPIRY_BUFFER } from './const';
import { ApiError } from './errors';
import type { ProviderType } from './providers';
import { AuthProviderRegistry } from './registry';
import { AuthBroadcastMessage, AuthState, AuthTokens, User } from './types';
...
export class JanAuthService {
    ...
    try {
      ...
      if (storedRefreshToken) {
        try {
          ...
          if (!this.getAuthProvider()) {
            console.log(
              'Session restored from tokens, but provider info was lost. Attempting to recover...',
            );
      ...
      if (tokenRefreshSucceeded && this.isAuthenticated()) {
        ...
        if (this.isAuthenticated()) {
          ...
          // This means the session is invalid - clear it
          this.clearAuthState();
        }
      }
    } catch (error) {
   ...
   * Handles authenticated tokens
   */
  async getValidAccessToken(): Promise<string> {
    await this.ensureInitialized();

    if (this.accessToken && Date.now() < this.tokenExpiryTime - TOKEN_EXPIRY_BUFFER) {
  ...
  async refreshAccessToken(): Promise<void> {
    try {
      const storedRefreshToken =
        this.refreshToken || localStorage.getItem(AUTH_STORAGE_KEYS.REFRESH_TOKEN);
      if (!storedRefreshToken) {
   ...
   */
  onAuthEvent(callback: (event: MessageEvent<{ type: AuthBroadcastMessage }>) => void): () => void {
  ...
  private async fetchUserProfile(): Promise<User | null> {
    try {
      return await this.makeAuthenticatedRequest<User>(`${JAN_BASE_URL}${AUTH_ENDPOINTS.ME}`);
    } catch (error) {
  ...
  /**
   * Update auth cookie for proxy compatibility
   */
  private updateAuthCookie(isLoggedIn: boolean): void {
    if (typeof window === 'undefined') return;
      ...
      const state = {
        ...
        user: this.currentUser
          ? {
              ...
              provider: this.getAuthProvider() || 'email',
              is_admin: this.currentUser.is_admin,
              roles: this.currentUser.roles,
            }
          : null,
      };
      Cookies.set(AUTH_COOKIE_NAME, JSON.stringify(state), {
        expires: 7, // 7 days
        path: '/',
      });
    } else {
...
export function getSharedAuthService(): JanAuthService {
  if (typeof window === 'undefined') return new JanAuthService(); // Server-side fallback
