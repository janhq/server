...
import { CHAT_STATUS, UPLOAD_STATUS } from "../lib/constants";
...
export type UploadServiceConfig = {
  ...
  blobUrlToDataUrl: (blobUrl: string) => Promise<string>;
  /** Upload media to server and get media ID */
  uploadMedia: (
    dataUrl: string,
    filename: string,
    userId: string,
  ...
  /** Create Jan media URL from media ID */
  createJanMediaUrl: (mediaId: string, mimeType: string) => string;
};
import {
  ArrowUp,
  Loader2Icon,
...
export type PromptInputControllerProps = {
  ...
  /** User ID for media upload tracking */
  userId?: string;
};
...
export type PromptInputProviderProps = PropsWithChildren<{
  ...
  /** Upload service configuration. If not provided, uploads will be skipped. */
  uploadService?: UploadServiceConfig;
...
export const usePromptInputIsUploading = () => {
  ...
  return attachments.files.some(
    (f) =>
      f.uploadStatus === UPLOAD_STATUS.PENDING ||
...
export const PromptInput = ({
  ...
  // Get uploadService and userId from provider if available, otherwise use props
  const effectiveUploadService = controller?.uploadService ?? uploadService;
  const effectiveUserId = controller?.userId ?? userId;
  ...
  useEffect(() => {
    // Skip upload if no upload service is configured
    if (!effectiveUploadService) return;

    const pendingFiles = files.filter(
      (f) =>
        f.uploadStatus === UPLOAD_STATUS.PENDING &&
        !uploadingFilesRef.current.has(f.id),
    );
    ...
    pendingFiles.forEach((file) => {
      ...
      (async () => {
        try {
          ...
          const dataUrl = await effectiveUploadService.blobUrlToDataUrl(
            file.previewUrl,
          );

          // Upload to media service
          const response = await effectiveUploadService.uploadMedia(
            dataUrl,
          ...
          const janMediaUrl = effectiveUploadService.createJanMediaUrl(
            response.id,
            response.mime,
          );
          updateFile(file.id, {
  ...
  }, [files, updateFile, effectiveUploadService, effectiveUserId]);
