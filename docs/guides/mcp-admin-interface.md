...
tools_to_disable = ["python_exec", "shell_command"]

for tool_key in tools_to_disable:
    requests.patch(
        f"http://localhost:8000/v1/admin/mcp/tools/{tool_key}",
        headers=headers,
        json={"enabled": False}
    )
    print(f"Disabled: {tool_key}")
```
...
# Apply safety filters to Python executor
filters = {
    "disallowed_keywords": [
        "^import os",
        "^import subprocess",
...
requests.put(
    "http://localhost:8000/v1/admin/mcp/tools/python_exec/filters",
    headers=headers,
    json=filters
)
...
- [MCP Tools API Reference](../api/mcp-tools/README.md) - Tool execution API
- [Authentication Guide](authentication.md) - Admin authentication
