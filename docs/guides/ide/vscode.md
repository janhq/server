...
```json
{
 ...
 "preLaunchTask": "Start Dev-Full Stack",
...
- `preLaunchTask`: Runs the task that executes `make dev-full`
...
|------|-------------|----------|
| **Start Dev-Full Stack** | Runs `make dev-full` (infra + services with host routing) | Native debugging |
| **Stop Dev-Full Stack** | Runs `make dev-full-stop` | Pause hybrid mode without removing containers |
| **Down Dev-Full Stack** | Runs `make dev-full-down` | Reset hybrid mode containers |
| **Start Full Stack** | Runs `make up-full` | Full Docker development/testing |
| **Stop All Services** | Runs `make down` | Stop and remove current stack |
| **Start Monitoring Stack** | Runs `make monitor-up` | Enable Prometheus/Grafana/Jaeger |
| **Stop Monitoring Stack** | Runs `make monitor-down` | Disable observability components |

### Service Tasks

| Task | Description | Make Equivalent |
|------|-------------|-----------------|
| **Run LLM API Locally** | Stop container + run `jan-cli dev run llm-api` | `./jan-cli.sh dev run llm-api` (or `.\jan-cli.ps1 ...`) |
| **Run MCP Tools Locally** | Stop container + run `jan-cli dev run mcp-tools` | `./jan-cli.sh dev run mcp-tools` |
...
| Task | Description | Make Equivalent |
|------|-------------|-----------------|
| **Run All Tests** | Run all integration tests | `make test-all` |
| **Run Unit Tests - LLM API** | Run API unit tests | `go test ./services/llm-api/...` |
| **Run Unit Tests - MCP Tools** | Run MCP unit tests | `go test ./services/mcp-tools/...` |
...
See [Makefile Commands](../development.md#makefile-commands-reference) for the full list of available targets.

**Most used:**
```bash
# Development
./jan-cli.sh dev run llm-api   # Run API natively (macOS/Linux)
./jan-cli.sh dev run mcp-tools # Run MCP natively (macOS/Linux)
.\jan-cli.ps1 dev run llm-api  # Windows equivalent
.\jan-cli.ps1 dev run mcp-tools
make dev-full      # Start dev-full stack
make dev-full-stop # Stop hybrid mode (containers stay up)

# Build & Test
make build-all # Build everything
make test-all # Run all integration suites
...
- [Makefile Commands](../development.md#makefile-commands-reference) - Make target cheat sheet
