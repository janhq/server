# New Service Guide (Template API)

This guide explains how to turn `services/template-api` into a real microservice and how to migrate existing services onto the shared scaffolding.
...
1. **Copy the template**
   ...
   ```
   or manually copy `services/template-api` to `services/my-service`.
...
4. **Configure the database**
   - Point `TEMPLATE_DATABASE_URL` at your PostgreSQL instance (rename the variable if desired).
...
8. **Smoke test**
   ...
   curl http://localhost:<port>/v1/sample
   ```
...
3. **Align config**:
   ...
   - Add service-specific sections (DB creds, upstream API keys, auth settings) and update env templates (`TEMPLATE_DATABASE_URL`, `AUTH_*`, etc.).
4. **Swap entrypoint**:
